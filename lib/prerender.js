...
const staticInstance = classOpts => {
  ...
  router.get(classOpts.endpoint, async (req, res) => {
    ...
    if (cache) {
      ...
      if (pageContent) {
        ...
        if (classOpts.afterRender) {
          classOpts.afterRender(sourceUserAgent, url, pageContent.content);
        }

        return res.status(pageContent.status).send(pageContent.content);
      }
    }
    ...
    pageContent = await browserCluster.execute(null, async ({ page }) => {
    ...
    if (classOpts.afterRender) {
      classOpts.afterRender(browserUserAgent, url, pageContent.content);
    }

    if (pageContent.redirectUrl) {
      return res.redirect(pageContent.status, pageContent.redirectUrl);
    } else {
      if (cache) {
        cache.set(url, pageContent.content);
      }
