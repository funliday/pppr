...
const staticInstance = classOpts => {
  ...
  router.get(classOpts.endpoint, async (req, res) => {
    ...
    if (!checkDomain) {
      console.error(`[DOMAIN] Not allowed domain: ${hostname} for ${url}`);
    ...
    const loadPage = async ({ page, data: { url, requestId } }) => {
      console.log(`[PUPPETEER-CLUSTER] Rendering: ${url}`);
      ...
      try {
        ...
        console.error(`[PUPPETEER-CLUSTER] Error: ${url} ${error}`);
    ...
    console.log(`[PUPPETEER-CLUSTER] Execute: ${url}`);
    ...
    return pageResponse(res, clusterResponse);
  });
  ...
  const handleRedirect = chain => {
    ...
    console.log(
      `[PUPPETEER-CLUSTER] Redirect: ${originalUrl} ${statusCode} to ${redirectUrl}`
    );
  ...
  const pageResponse = (
    res,
    { url, status, content, redirectUrl, requestId }
  ) => {
    if (classOpts.afterRender) {
