 ...
 * @param {object} [opts.cache={}] LRU cache options
 * @param {number} [opts.cache.max=50] - LRU cache entry max count
 * @param {number} [opts.cache.maxAge=30000] - LRU cache entry max age (milliseconds)
...
const staticInstance = opts => {
  ...
  const cache = new LRU(opts.cache);
...
const decorateOptions = opts => {
  ...
  opts.cache.maxAge = opts.cache.maxAge || 1000 * 60 * 5; // 5 mins
