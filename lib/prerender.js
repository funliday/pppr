...
const staticInstance = classOpts => {
  ...
  router.get(classOpts.endpoint, async (req, res) => {
    ...
    const loadPage = async ({ page, data: { url, requestId } }) => {
      console.log(`[PUPPETEER-CLUSTER] rendered url: ${url}`);
    ...
    console.log(`[PUPPETEER-CLUSTER] execute url: ${url}`);
  ...
  const handleRedirect = chain => {
    ...
    console.log(
      `[PUPPETEER-CLUSTER] from ${originalUrl} ${statusCode} to ${redirectUrl}`
    );
